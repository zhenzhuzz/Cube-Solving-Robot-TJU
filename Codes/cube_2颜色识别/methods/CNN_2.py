...
from methods.getdata import data_for_cnn_2 as data_for_cnn
from methods.getdata import data_split
import numpy as np
import tensorflow
...
#                  batch_size=32,     # 用多大量的数据计算直方图
	...
	path = './models/cnn_fin_2_{time}.h5'.format(time=str(time())[:-8])
	model.save(path)
	return path
...
def train_cnn_main():
	...
	if Input == b'\r':
		...
		with open('./models/cnn_model_2.txt', 'w') as file:
